...
class Lugar(BaseEntity):
    __tablename__ = "lugar"
    __table_args__ = (
    db.UniqueConstraint('pais_id', 'ciudad_id', 'barrio_id', 'provincia_id', name='uidx_sitio_unico'),)
    id = db.Column(db.Integer, primary_key=True)
...
class Carrera(BaseEntity):
    ...
    __table_args__ = (db.UniqueConstraint('programa_id', 'facultad_id', 'universidad_id', 'campus_id',
                                          name='uix_table_programa_facultad_universidad_campus'),)
    id = db.Column(db.Integer, primary_key=True)
