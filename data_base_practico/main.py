...
for _, datos in personasDF.iterrows():
	...
	try:
		...
		lugar = Lugar.crear_y_obtener(session_db, pais=pais , barrio=barrio , provincia=provincia , ciudad=ciudad) 
		...
		persona = Persona.crear_y_obtener(session_db, nombre=fila['first_name'], apellido=fila['last_name'], email=fila['email'], birthdate=fila['birthdate'], personal_id=fila['personal_id'], lugar=lugar, genero=genero)
		session_db.commit()
	except:
		session_db.rollback()
...
for _, datos in cursos_profesoresDF.iterrows():
	...
	try:
		...
		universidad=Universidad.crear_y_obtener(session_db, nombre=fila['institute'])
		campus=Campus.crear_y_obtener(session_db, nombre=fila['campus'])
		carrera = Carrera.crear_y_obtener(session_db, programa=programa, facultad=facultad, universidad=universidad, campus = campus)
		rol=TipoPersona.crear_y_obtener(session_db, nombre='Profesor')
		persona=Persona.crear_y_obtener(session_db, personal_id=fila['instructor'])
		persona_carrera = personasCarreras.crear_y_obtener(session_db, persona=persona, carrera=carrera, tipopersona=rol)
		session_db.commit()
	except:
		session_db.rollback()
...
for _, datos in alumnosDF.iterrows():
	...
	session_db.begin()
	try:
		persona=Persona.crear_y_obtener(session_db, personal_id=fila['personal_id'])
		programa=Programa.crear_y_obtener(session_db, nombre=fila['program'])
